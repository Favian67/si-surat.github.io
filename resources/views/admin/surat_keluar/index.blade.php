...
@section('content')
    <h2 class="text-2xl font-bold mb-4">Surat Keluar</h2>
    <a href="{{ route($rolePrefix . '.surat-keluar.create') }}"
            ...
            <tbody>
                @forelse($surat as $i => $s)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="border p-2">{{ \Carbon\Carbon::parse($s->tanggal_dikirim)->format('d M Y H:i') }}</td>
                        <td class="border p-2">
                            @if($s->file)
                            ...
                            @endif
                        <td class="border p-2">
                            <a href="{{ route($rolePrefix . '.surat-keluar.edit', $s->id) }}"
                                class="bg-yellow-500 text-white px-2 py-1 rounded">Edit</a>
                            <form method="POST" action="{{ route($rolePrefix . '.surat-keluar.destroy', $s->id) }}"
                                class="inline-block">
                                @csrf @method('DELETE')
